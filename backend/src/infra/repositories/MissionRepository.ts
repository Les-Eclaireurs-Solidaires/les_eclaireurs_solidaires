...
export class MissionRepository implements IMissionRepository {
  ...
  public async updateOrganizers(
    ...
    try {
      const deleteOrganizerMission = `DELETE FROM mission_organizer WHERE id_mission = (SELECT mission_id FROM mission WHERE mission_uuid = ?)`;
  ...
  private async hydrateRegistrations(
    ...
    const queryRegistration = `SELECT
                                registration_id AS id,
                                registration_date AS date,
                                registration_recall_send_at AS recallSendAt, 
                                id_registration_status AS status,
                                user.user_uuid AS volunteerUuid
                                FROM registration
                                LEFT JOIN \`user\` ON registration.id_user = user.user_id
                                WHERE id_mission = (SELECT mission_id FROM mission WHERE mission_uuid = ?)
                                ORDER BY registration_date DESC`;
    const [resultRegistration] = await db.execute<RowDataPacket[]>(
      queryRegistration,
      [missionUuid],
    );

    if (resultRegistration.length === 0) return [];

    return resultRegistration.map(
      (row) =>
        new Registration(
          {
            date: row.date,
